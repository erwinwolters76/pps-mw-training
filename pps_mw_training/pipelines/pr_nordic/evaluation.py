...
def evaluate_model(
    ...
    for idx in range(mw_data.time.size):
        ...
        plt.title("Radar")
        plt.subplot(2, 3, 4)
        plt.imshow(pred[idx, :, :, 3], cmap="coolwarm", vmin=-30, vmax=30)
        plt.title("0.25")
        plt.colorbar()
        plt.subplot(2, 3, 5)
        plt.imshow(pred[idx, :, :, 4], cmap="coolwarm", vmin=-30, vmax=30)
        plt.colorbar()
        plt.title("0.5")
        plt.subplot(2, 3, 6)
        plt.imshow(pred[idx, :, :, 5], cmap="coolwarm", vmin=-30, vmax=30)
        plt.colorbar()
        plt.title("0.75")
        plt.show()
